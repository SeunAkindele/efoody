	...
	return (
		<SafeArea>
			...
			<StaffContainer>
				...
				<Spacer size="large">
					<Dropdown data={locations} onValueChange={(item) => setLocation(item)} placeholder="Select Location *" />
				</Spacer>
				<Spacer size="large">
					<Dropdown data={position} onValueChange={(item) => setType(item)} placeholder="Select Position *" />
				</Spacer>

				<Spacer size="large">
					<StaffButton mode="contained" onPress={() => createStaff(name, email, phone, type, location, navigation)}>
						Create Staff
					</StaffButton>
				</Spacer>
			</StaffContainer>
