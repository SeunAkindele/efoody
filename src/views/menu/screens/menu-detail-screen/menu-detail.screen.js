...
import { MenuInfoCardDetails } from "../../components/menu-info-card-details/menu-info-card-details-component";
...
export const MenuDetailScreen = ({ route, navigation }) => {
	...
	return (
		<>
			<MenuInfoCardDetails menu={menu} />
			<ScrollView style={{ backgroundColor: "#fff" }}>
				{menu.ingredients && strlen(menu.ingredients) > 0 ? (
					<List.Accordion title="Ingredients" left={(props) => <List.Icon {...props} icon="bread-slice" />} expanded={breakfastExpanded} onPress={() => setBreakfastExpanded(!breakfastExpanded)}>
			...
			<Spacer position="bottom" size="large">
				<OrderButton onPress={() => addToCart(menu.id, navigation)} mode="contained">
					ADD TO CART
				</OrderButton>
			</Spacer>
		</>
